...
-- Version 2.1: richer vacancies and multi-channel publication workflow.
...
create table if not exists public.job_publications (
  ...
  portal text not null, status text not null default 'draft' check (status in ('draft','ready','published','paused','error')),
  external_url text, external_id text, published_at timestamptz, last_error text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(job_id, portal)
);
